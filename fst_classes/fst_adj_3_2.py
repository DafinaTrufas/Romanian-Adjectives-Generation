...
class Adj3_2_tor(InflectionalClass):
    ...
    def to_fsg(self) -> pn.Fst:
        return (self.fst + pn.accep(self.stem[-3]) + pn.cross(self.stem[-2], "oa") + pn.accep(self.stem[-1]) + pn.accep('e')).optimize()
...
class Adj3_2_âu(InflectionalClass):
    ...
    @dec_letter_mapping
    def to_fsg(self) -> pn.Fst:
        return (self.fst + pn.accep(self.stem[-2], token_type="utf8") + pn.cross(self.stem[-1], "i") + pn.accep('e')).optimize()
    
    @dec_letter_mapping
    def to_mpl(self) -> pn.Fst:
        return (self.fst + pn.accep(self.stem[-2], token_type="utf8")  + pn.cross(self.stem[-1], 'i')).optimize()
...
class Adj3_2_ău(InflectionalClass):
    ...
    @dec_letter_mapping
    def to_fsg(self) -> pn.Fst:
        return (self.fst + pn.cross(get_ro_fst(self.stem[-2]), "a") + pn.cross(self.stem[-1], "ie")).optimize()
