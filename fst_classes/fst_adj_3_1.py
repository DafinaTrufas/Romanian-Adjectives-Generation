...
class Adj3_1_esc(InflectionalClass):
    ...
    def to_fsg(self) -> pn.Fst:
        return (self.fst + pn.cross(self.stem[-3], "ea") + pn.accep(self.stem[-2:]) + get_ro_fst("ă")).optimize()
    
    @dec_letter_mapping
    def to_mpl(self) -> pn.Fst:
        return (self.fst + pn.accep(self.stem[-3]) + pn.cross(self.stem[-2], get_ro_fst("ș")) + pn.cross(self.stem[-1], get_ro_fst("t") + pn.accep('i'))).optimize()
